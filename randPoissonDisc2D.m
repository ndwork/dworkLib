...
function [x,y] = randPoissonDisc2D( r, varargin )
  % RANDPOISSONDISC2D Generate Poisson-disc-sampled points in two dimensions
  %
  %   [X,Y] = RANDPOISSONDISC2D(R) randomly generates a collection of points (X,Y)
  %     in rectangle (-0.5,-0.5),(0.5,0.5) that satisfy the minimum-distance 
  %     requirement specified by R using the Brindson method (without using the
  %     background grid).
  %
  %   RANDPOISSONDISC2D uses the RAND function in its current/existing state.
  %
