...
function [H,inlierIndxs,outlierIndxs] = ransacDltHomographyFromPts3D( ...
  ...
  %
  % Inputs:
  % pts1/pts2 - 2D arrays of size Mx3
  ...
  while nRansac > sample_count
    ...
    indxs = randsample(nPts,5);
    subset1 = pts1(indxs,:);
    subset2 = pts2(indxs,:);
    H = dltHomographyFromPts3D( subset1, subset2 );
  ...
  H = dltHomographyFromPts3D( pts1(inlierIndxs,:), pts2(inlierIndxs,:) );
end
